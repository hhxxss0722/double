...
def create_asc_file(root_path, src_path_compoent, output_pre_path, dbc_list, group_infor_dic, stated_sig_display_dic):
    ...
    for dbc in dbc_list:
        for mode in value_mode_list:
            start_time = 0.000000
            output_path = output_pre_path + '\\' + dbc.file_name + '_' + mode.lower() +'.asc'
            _create_specified_asc_file(pre_f_path, output_path, dbc.message_list, mode, start_time)
...
def _create_specified_asc_file(pre_f_path, output_path, msg_list, value_mode, start_time):
...
def _set_cmd_str(cmd_str, sig, target_value):
    ...
    while cnt < byte_length:
        ...
        try:
            data = '{:0>2}'.format(str(hex(int(''.join(tt_list), 2)))[2:])      ##右对齐 b2
        except:
            raise Exception
...
def _set_full_byte(cmd_str, data_str, ordering, byte_sum, byte_index):
    ...
    if ordering:
        i = 0
        while(i < byte_sum):
            cmd_data_str = cmd_list[int(byte_index + byte_sum - 1 - i)]
            ...
            try:
                ...
                pass
            cmd_list[int(byte_index + byte_sum - 1 - i)] = cmd_data_str
            i += 1
    else:
        i = 0
        while(i < byte_sum):
...
def _get_random_val(sig):
    ...
    if sig.name in random_val_dict.keys():
