...
def create_sig_c_file(sig_c_src_path_prefix, sig_c_output_path, dbc_list, valid_sig_name_list, different_unit_path,
    ...
    str1 += r'/* for print */' + '\n'
    str1 += '#define printd  if(0)rt_printf_can' + '\n'
    tar_f.write(str1)
    ...
    for dbc in dbc_list:
        ...
        for message in dbc.message_list:
            ...
            str1 += '#define CAN_' + message.name.upper() + '_OUTIND\t\t\t' + 'canstack_timeout_array' + str(dbc.can_channel) + '[' + message.name.upper() + '].outind'
            str1 += '\n\n'
    ...
    if len(different_unit_list) != 0:
        ...
        d_f = open(different_unit_path, 'wb+')
