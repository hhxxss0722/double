...
class CanDev(object):
    ...
    def ReceiveData(self):
        print('receivedata enter')
        robj=Canobj()
        # robj= ( Canobj * 20) ()
        lRel = self.receivedata(self.devtype,self.devindex,self.canindex,byref(robj),20,50)
        print('receivedata over')
        print('robj.id = '+ str(robj.ID))
        print('list(robj.Data)='+str(list(robj.Data)))
        rmsg={}
        rmsg['slen']=lRel
        rmsg['obj']=robj
        return rmsg
            ...
            try:
                ...
                if rnum==0:
                    ...
                    for i in range(0,rnum):
                        ...
                        # print('self.cdev.ReceiveData() = ' +str(rdata))
                        # print('rdata[\'slen\']= ' + str(rdata['slen']))
                        if rdata['slen']==0:
                            pass
                        else:
                            slen = rdata['slen']
                            ...
                            for i in range(0,slen):
                                ...
                                dlist=[]
                                dlist.append(robj.ID)
                                print('robj[i].ID = '+str(robj.ID))
                                dlist.append(self.ToBytes(robj.Data))
