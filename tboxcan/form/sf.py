...
class Sendwindow(QtWidgets.QWidget):
	...
	def __init__(self,com):  
		...
		self.com=com
		self.new.btn_send.clicked.connect(self.senddata)
		self.new.btn_close.clicked.connect(self.closewindow)
		self.new.btn_clear.clicked.connect(self.cleardata)
		
	def cleardata(self):
		self.new.txt_send.setText("")
		
	def senddata(self):
		try:
			text=self.new.txt_send.toPlainText()
			text+="\r\n"
			if self.new.cb_16.isChecked():
				...
				self.com.write(text.encode("utf-8"))
	...
	def ShowBox(self,s):
		reply = QMessageBox.information(self, "发送错误", s,QMessageBox.Yes)
