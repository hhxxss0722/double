            ...
            if len(temp_line) == 0:
                ...
                if (len(sig_name) > 0) and is_valid:
                    try:
                        ...
                        if len(sig_unit) > 0:
                            x = chardet.detect(sig_unit)	#chardet判断字符编码方法
                ...
                if cnt + 1 <= dbc_line_sum:
                    ...
                    if len(temp_line) == 0:
                        ...
                        # dbc默认所有[未对应有效报文的]信号属于一个默认的报文——vector__independent_sig_msg，ID=0x0
                        # current message's ID is invalid or sig list is empty, delete current message
                        if int(message1.ID, 16) == 0 or len(sig_list) == 0:
                            message_list.pop()	#pop() 函数用于移除列表中的一个元素（默认最后一个元素），并且返回该元素的值。
                        elif message1.dlc > 8:
