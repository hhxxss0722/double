...
def SaveSig(sig_infor,erow):
	...
	if d_sig_infor['mes_name'] in LST_MES_NAME:
		sig_str = ' SG_ ' + d_sig_infor['sig_name'] + ' : ' + d_sig_infor['startbit'] + '|' + d_sig_infor['sig_len'] + '@' + '1-' + ' (' + str(d_sig_infor['formula'][0]) + ',' + str(d_sig_infor['formula'][1]) + ') ' + '[' + str(d_sig_infor['phy_scale'][0]) + '|' + str(d_sig_infor['phy_scale'][1]) + '] "' + d_sig_infor['unit'] + '" Vector__XXX' + '\n'
		print sig_str
	else:
...
class SIGNAL(object):		  
	...
	def write_sig(self,byte_order):
		...
		# self.is_signed = 
		sig_str = ' SG_ ' + self.sig_name + ' : ' + str(start_bit_conv) + '|' + str(self.sig_len) + '@' + str(self.byte_order) + self.is_signed + ' (' + str(self.factor) + ',' + str(self.offset) + ') ' + '[' + str(self.sig_min) + '|' + str(self.sig_max) + '] "' + self.sig_unit + '" Vector__XXX' + '\n'		#self.sig_unit.encode('gb2312')
		# print sig_str
		return sig_str
...
class MESSAGE(object):	
	...
	def write_mes(self):
	   ...
	   lst_other = []
	   self.mes_id_conv()
	   mes_str = '\n' + 'BO_ ' + str(self.mes_id) + ' ' + self.mes_name + ': ' + str(self.mes_len) + ' Vector__XXX' + '\n'
...
def GetLine(filename,newfile,pre_dbc_1,pre_dbc_2):
	...
	for i in xrange(0,nrows):
		...
		if i < HEADERROW:
		...
		elif table.row(i)[MESCOL].value == u'报文名称':
			m_row = i
			for j,item in enumerate(rowValues):
			...
			if m_row == 0 and table.row(i)[SIGCOL].value == 'YES':
				...
				d_sig['sig_name'] = SIGNAL(d_sig['mes_name'],d_sig['sig_name'],d_sig['start_bit'],d_sig['sig_len'],d_sig['sig_formula'],d_sig['phy_scale'],d_sig['sig_unit'],d_sig['sig_state'])
				...
				d_mes['mes_name'] = MESSAGE(d_mes['mes_name'],d_mes['mes_id'],d_mes['mes_cycle'],d_mes['mes_len'],d_mes['byte_order'])
				curr_mes = d_mes['mes_name']
				(mes_str,lst_other) = curr_mes.write_mes()
				newfile.write(mes_str)	#写报文行		
				lst_others.extend(lst_other)
				...
				for obj in sig_obj: 	
					if curr_mes.mes_name == obj.mes_name:
						...
						if obj.value_tables != '':
							vts_str = obj.value_table_split(curr_mes.mes_id)	#存储value tables
							print vts_str
...
def CreatPath(file1,path):
	...
	new_txt = os.path.join(new_path, dbc_name)
	...
	GetLine(file1,outfile,pre_dbc_1,pre_dbc_2)
